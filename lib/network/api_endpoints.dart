...
class ApiEndpoints {
  Future<List<PokemonsModel>?> getPokemons() async {
    var URL = Uri.parse('https://pokeapi.co/api/v2/pokemon?limit=1118');
  ...
  Future<List<PokemonTypesModel>?> getPokemonTypes(int id) async {
    ...
    if (response.statusCode == 200) {
      ...
      List<PokemonTypesModel> pokemonData = pokemon
          .map((pokemonTypes) => PokemonTypesModel.fromMap(pokemonTypes))
          .toList();
      return pokemonData;
    } else {
      return null;
  ...
  Future<PokemonColorModel?> getPokemonColor(int id) async {
    ...
    if (response.statusCode == 200) {
      ...
      PokemonColorModel pokemonColor =
          PokemonColorModel(name: pokemon['name'], url: pokemon['url']);
      return pokemonColor;
    } else {
      return null;
    }
